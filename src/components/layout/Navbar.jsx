  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-40 md:top-0 md:bottom-auto bg-dark-900/90 backdrop-blur-lg border-t md:border-t-0 md:border-b border-dark-700/50 safe-area-bottom">
      <div className="max-w-7xl mx-auto px-4 flex items-center justify-between h-16">
        ...
        <div className="flex items-center justify-start md:justify-end gap-1 w-full md:w-auto overflow-x-auto scrollbar-none flex-nowrap px-1">
          <button
            onClick={onSearchOpen}
            className="flex md:hidden flex-col items-center gap-1 px-3 py-2 rounded-xl text-dark-400 hover:text-dark-200 hover:bg-dark-800/50 transition-all flex-shrink-0"
            title="Search"
          >
          ...
          {navItems.map(({ to, icon: Icon, label }) => (
            <NavLink
              ...
              className={({ isActive }) =>
                `flex flex-col md:flex-row items-center gap-1 px-3 py-2 rounded-xl transition-all duration-200 flex-shrink-0 ${
                  isActive
          ...
          <button
            onClick={onSync}
            disabled={syncing}
            className="flex flex-col md:flex-row items-center gap-1 px-3 py-2 rounded-xl text-dark-400 hover:text-dark-200 hover:bg-dark-800/50 transition-all disabled:opacity-50 flex-shrink-0"
